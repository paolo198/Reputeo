...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const sub of subs) {
      ...
      const reportDateLabel = reportDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })

      await resend.emails.send({
        from: 'Reputeo <noreply@reputeo.app>',
        to: user.email,
        subject: `📊 Votre rapport Reputeo — ${reportDateLabel}`,
        html: `
          <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;background:#080810;color:#f0f0ff;padding:40px;border-radius:16px">
            ...
            <p style="color:rgba(255,255,255,0.4);margin-bottom:8px">Rapport hebdomadaire</p>
            <p style="color:rgba(255,255,255,0.65);font-size:0.82rem;margin:0 0 32px">Rapport généré le ${reportDateLabel}</p>
            ...
            ${reviews.slice(0, 3).map(r => `
              <div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:14px;margin-bottom:10px">
                <div style="font-weight:500;font-size:0.875rem;margin-bottom:4px">${escapeHtml(String(r.author || 'Client').slice(0, 120))}</div>
                <div style="color:rgba(255,255,255,0.5);font-size:0.82rem">${escapeHtml(String(r.text || '').replace(/[\u0000-\u001f\u007f]/g, ' ').slice(0, 100))}</div>
              </div>`).join('')}
      ...
      await sb.auth.admin.updateUserById(user.id, { user_metadata: { ...(user.user_metadata || {}), last_weekly_report_at: reportDate.toISOString() } })
    ...
    res.status(500).json({ error: 'Impossible de générer les rapports pour le moment.' })
  }
}
